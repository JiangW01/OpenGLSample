...
inline fun loadBitmapFromAssets(context: Context, fileName: String): Bitmap {
    val inputStream = context.assets.open(fileName)
...
/**
 * 加载纹理
 * @param isOes 加载纹理方式 默认值 false
 *        true GLES11Ext.GL_TEXTURE_EXTERNAL_OES android解码用这个
 *        false GLES20.GL_TEXTURE_2D
 * @param
 */
inline fun loadTexture(
    ...
    var target = GLES20.GL_TEXTURE_2D
    if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.ICE_CREAM_SANDWICH_MR1) {
        target = if (isOes) GLES11Ext.GL_TEXTURE_EXTERNAL_OES else GLES20.GL_TEXTURE_2D
    }
    ...
    //设置s轴拉伸方式---重复
    GLES20.glTexParameterf(target, GLES20.GL_TEXTURE_WRAP_S, renderTypeS.toFloat())
    //设置t轴拉伸方式---重复
    GLES20.glTexParameterf(target, GLES20.GL_TEXTURE_WRAP_T, renderTypeT.toFloat())
    return textureId
}
