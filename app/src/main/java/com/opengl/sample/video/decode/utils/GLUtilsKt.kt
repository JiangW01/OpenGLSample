package com.video.decode.utils
 ...
 * @return opengl程序句柄
 */
inline fun glCreateProgram(vertexShaderSource: String, fragmentShaderSource: String): Int {
    val vertexShader = loadVertexShader(vertexShaderSource)
    val fragmentShader = loadFragShader(fragmentShaderSource)
...
inline fun glEnableVertexAttribPointer(
    handle: Int,dataBuffer: Buffer, size: Int = COORDS_VERTEX_3D,
    type: Int = GLES20.GL_FLOAT, normalized: Boolean = false, stride: Int = size * 4
): Int {
    GLES20.glVertexAttribPointer(handle, size, type, normalized, stride, dataBuffer)
    GLES20.glEnableVertexAttribArray(handle)
